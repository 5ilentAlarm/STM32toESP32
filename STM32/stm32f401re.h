...
static inline void uart1_init(unsigned long baud) {
    ...
    uint8_t af = 7; //Set af to UART
    uint16_t rx = 0, tx = 0;

    RCC -> APB2ENR |= BIT(4); //enable bit 7 of APB2ENR register for USART1
    tx = PIN('B', 6);
    rx = PIN('A', 10);
    ...
    UART1 -> CR1 = 0; //disable UART1 control registers
    UART1 -> BRR = FREQ / baud; //16Mhz divided by the baud rate
    UART1 -> CR1 |= BIT(13) | BIT(2) | BIT(3);
}   
